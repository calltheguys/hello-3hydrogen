...
import {SVGRenderer as SVGRendererLib} from 'three-stdlib';
...
function Canvas({children, resize, style, className, ...props}) {
  ...
  const [gl] = useState(() => new SVGRendererLib());
  const root = React.useRef(null);

  if (size.width > 0 && size.height > 0) {
    if (!root.current) root.current = createRoot(ref.current);
